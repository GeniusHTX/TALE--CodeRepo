...
from repo_agent.utils.write_to_jsonl import write_to_jsonl
class ChatEngine:
    """
    ...
    def generate_doc(self, doc_item: DocItem):
        ...
        messages = self.build_prompt(doc_item) # 封装了prompt，返回List[BaseMessage] 对象,system+user，每个BaseMessage.content是文本
        ...
        try:
            ...
            write_to_jsonl("llm_response.jsonl",{"LLM Prompt Tokens": response.raw.usage.prompt_tokens,
            "LLM Completion Tokens": response.raw.usage.completion_tokens,
            "Total LLM Token Count": response.raw.usage.total_tokens})
...
def extract_number(text):
    ...
    if match:
        result = match.group(1)  # 提取捕获组中的数字
